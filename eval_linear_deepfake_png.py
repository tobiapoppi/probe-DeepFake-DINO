    ...
    if args.evaluate:
        ...
        if args.transforms:
            ...
            for k, v in transformations_list.items():
                ...
                transform_dataset_val = datasets.ImageFolder(os.path.join(png_dataset_path, "test"), transform=val_transform)
                transform_val_loader = torch.utils.data.DataLoader(
                    transform_dataset_val,
                    batch_size=args.batch_size_per_gpu,
                    num_workers=args.num_workers,
                    pin_memory=True,
                )
